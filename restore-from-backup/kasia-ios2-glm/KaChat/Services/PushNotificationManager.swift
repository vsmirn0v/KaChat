...
final class PushNotificationManager: ObservableObject {
    ...
    private let appAttestKeyIdDefaultsKey = "push_app_attest_key_id"
    private let appAttestEnrolledDefaultsKey = "push_app_attest_enrolled"

    private var deviceToken: String?
    private var appAttestKeyId: String?
    private var appAttestEnrolled = false
    ...
    private var appAttestRecoveryRetryPending = false
    ...
    func registerWithIndexer() async throws {
        ...
        for attempt in 0..<2 {
            ...
            if httpResponse.statusCode == 200 {
                ...
                markAppAttestEnrolledIfNeeded(auth)
                appAttestRecoveryRetryPending = false
            ...
            handleAppAttestServerHint(reason)
            if isAppAttestAssertionFailure(statusCode: httpResponse.statusCode, reason: reason),
               attempt == 0 {
                appAttestRecoveryRetryPending = true
    ...
    private func runWatchedUpdate() async {
        ...
        do {
            ...
            guard httpResponse.statusCode == 200 else {
                ...
                handleAppAttestServerHint(reason)
                if isAppAttestAssertionFailure(statusCode: httpResponse.statusCode, reason: reason),
                   !appAttestRecoveryRetryPending {
                    appAttestRecoveryRetryPending = true
                    lastWatchedSignature = nil
                    Task { [weak self] in
                        try? await Task.sleep(nanoseconds: 400_000_000)
                        await self?.runWatchedUpdate()
                    }
                    NSLog("[Push] App Attest assertion rejected for watched update, retrying")
                    return
                }
    ...
    private func loadPersistedState() {
        ...
        appAttestKeyId = UserDefaults.standard.string(forKey: appAttestKeyIdDefaultsKey)
        appAttestEnrolled = UserDefaults.standard.bool(forKey: appAttestEnrolledDefaultsKey)
        if appAttestKeyId == nil {
            appAttestEnrolled = false
        }
    }
    ...
    private func persistAppAttestKeyId(_ keyId: String) {
        appAttestKeyId = keyId
        UserDefaults.standard.set(keyId, forKey: appAttestKeyIdDefaultsKey)
    }

    private func persistAppAttestEnrolled(_ enrolled: Bool) {
        appAttestEnrolled = enrolled
        UserDefaults.standard.set(enrolled, forKey: appAttestEnrolledDefaultsKey)
    }

    private func resetAppAttestEnrollment() {
        persistAppAttestEnrolled(false)
    }
    ...
    private func buildPushAuth(
        ...
        let appAttestProof = try await buildAppAttestProof(challenge: preimage)
        ...
        return PushAuthRequest(
            ...
            appAttestKeyId: appAttestProof?.keyId,
            appAttestAttestation: appAttestProof?.attestationB64,
            appAttestAssertion: appAttestProof?.assertionB64
        )
    }
    ...
    private struct AppAttestProof {
        let keyId: String
        let attestationB64: String?
        let assertionB64: String?
    }

    private func buildAppAttestProof(challenge: String) async throws -> AppAttestProof? {
        ...
        let clientDataHash = Data(CryptoKit.SHA256.hash(data: Data(challenge.utf8)))
        var keyId = try await ensureAppAttestKeyId()

        if !appAttestEnrolled {
            do {
                ...
                // Existing key may be unusable for enrollment on server side; rotate and enroll a fresh one.
                keyId = try await DCAppAttestService.shared.generateKey()
                persistAppAttestKeyId(keyId)
                persistAppAttestEnrolled(false)
                let attestation = try await DCAppAttestService.shared.attestKey(
                    keyId,
                    clientDataHash: clientDataHash
        ...
        do {
            ...
            // Key material may come from a different App Attest environment (dev/prod switch).
            // Rotate and re-enroll immediately so the backend can bind a valid key.
            keyId = try await DCAppAttestService.shared.generateKey()
            persistAppAttestKeyId(keyId)
            persistAppAttestEnrolled(false)
            let attestation = try await DCAppAttestService.shared.attestKey(
    ...
    private func ensureAppAttestKeyId() async throws -> String {
        if let existing = appAttestKeyId,
        ...
        throw PushError.authFailed(reason: "App Attest is not available on this iOS version")
    }
    ...
    private func markAppAttestEnrolledIfNeeded(_ auth: PushAuthRequest) {
        if auth.appAttestAttestation != nil {
            persistAppAttestEnrolled(true)
        }
    }

    private func handleAppAttestServerHint(_ reason: String?) {
        guard let reason else { return }
        let normalized = reason.lowercased()
        if normalized.contains("app_attest") && normalized.contains("enrollment") {
            resetAppAttestEnrollment()
        }
        if normalized.contains("app attest") && normalized.contains("enrollment") {
            resetAppAttestEnrollment()
        }
        if normalized.contains("app_attest") && normalized.contains("assertion") {
            resetAppAttestEnrollment()
        }
        if normalized.contains("app attest") && normalized.contains("assertion") {
            resetAppAttestEnrollment()
        }
        if normalized.contains("assertion signature") {
            resetAppAttestEnrollment()
        }
    }

    private func isAppAttestAssertionFailure(statusCode: Int, reason: String?) -> Bool {
        guard statusCode == 401, let reason else { return false }
        let normalized = reason.lowercased()
        if normalized.contains("invalid app attest assertion signature") {
            return true
        }
        if normalized.contains("invalid app_attest assertion signature") {
            return true
        }
        if normalized.contains("app attest") && normalized.contains("assertion") {
            return true
