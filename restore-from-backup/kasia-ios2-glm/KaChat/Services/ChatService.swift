...
final class ChatService: ObservableObject {
    ...
    /// Used after manually adding a new contact so old messages are not skipped by global cursors.
                ...
                for alias in pollAliases {
                    do {
                        let messages = try await KasiaAPIClient.shared.getContextualMessagesBySender(
                            ...
                            alias: alias,
                            limit: 50,
                            blockTime: applyMessageRetention(to: lastPollTime)
                        )
    ...
    private func fetchHandshakesOnly() async {
        ...
        let since = lastPollTime > 60_000 ? lastPollTime - 60_000 : lastPollTime
        let privateKey = WalletManager.shared.getPrivateKey()

        NSLog("[ChatService] Fetching incoming handshakes (since=%llu)...", since)
        // Fetch incoming handshakes
        guard let incoming = await retryUntilSuccess(
            label: "fetch incoming handshakes (bootstrap)",
            operation: { [self] in try await fetchIncomingHandshakes(for: wallet.publicAddress, blockTime: since) }
        ) else {
        ...
        guard let outgoing = await retryUntilSuccess(
            ...
            operation: { [self] in try await fetchOutgoingHandshakes(for: wallet.publicAddress, blockTime: since) }
        ) else {
    ...
    func fetchNewMessages(forActiveOnly activeAddress: String? = nil) async {
        ...
        // Fetch handshakes first (they establish aliases)
        // Subtract 60s from lastPollTime to cover possible blockchain reorgs
        let reorgBufferMs: UInt64 = 60_000
        let since = lastPollTime > reorgBufferMs ? lastPollTime - reorgBufferMs : lastPollTime
        let messageSince = applyMessageRetention(to: since)
        ...
        if shouldFetchPayments {
            ...
            if !inPayments.isEmpty || !outPayments.isEmpty {
                ...
                lastPaymentFetchTime = since > 0 ? since : UInt64(Date().timeIntervalSince1970 * 1000)
            }
    ...
    private func fetchContextualMessages(myAddress: String, privateKey: Data?, since: UInt64) async -> Bool {
        let effectiveSince = applyMessageRetention(to: since)
        ...
        print("[ChatService] Fetching contextual messages for \(allContactAddresses.count) contacts since \(effectiveSince)")
    ...
    private func fetchContextualMessagesForActive(
        ...
        let effectiveSince = forceExactBlockTime ? since : applyMessageRetention(to: since)
        beginChatFetch(contactAddress)
        var contactSuccess = true
        defer { endChatFetch(contactAddress, success: contactSuccess) }
    ...
    private func fetchContextualMessagesFromContact(contactAddress: String, myAddress: String, privateKey: Data) async -> ContactFetchResult {
        ...
        let since = applyMessageRetention(to: lastPollTime > 0 ? lastPollTime - 60000 : 0)

        var newMessagesAdded = false

        do {
            for alias in aliases {
                let fetchKey = contextualFetchKey(address: contactAddress, alias: alias, limit: 10, since: since)
                guard beginContextualFetch(fetchKey) else {
                ...
                let messages = try await apiClient.getContextualMessagesBySender(
                    ...
                    blockTime: since  // Look back 1 minute from last poll
                )
...
private struct TransactionResolveInfo {
    let sender: String
    let blockTimeMs: UInt64
    let payload: String?
}

private struct KaspaTransactionResponse: Codable {
    let outputs: [KaspaTransactionOutput]
}

private struct KaspaTransactionOutput: Codable {
...
private struct DecryptedHandshake: Codable {
    let alias: String?
    let type: String?
...
private struct PendingSelfStash: Codable, Identifiable, Equatable {
    let id: UUID
...
private struct KaspaFullTransactionResponse: Codable {
...
private struct KaspaFullTxInput: Codable {
...
private struct FlexibleUInt64: Codable {
    let value: UInt64
...
private struct FlexibleUInt8: Codable {
    let value: UInt8
...
private struct KaspaFullTxOutput: Codable {
    let transactionId: String?
    let index: Int?
    let amount: UInt64
...
private actor InFlightResolveTracker {
