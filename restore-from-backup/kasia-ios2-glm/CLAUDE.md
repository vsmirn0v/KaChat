...
Kasia is a native iOS messaging and payment application built on the Kaspa blockchain. It enables encrypted peer-to-peer messaging with on-chain payments using SwiftUI.
...
4. **lastPollTime**: Track last synced block time to fetch only new data
...
- Triggers message/payment sync on subscription restart
