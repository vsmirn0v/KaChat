...
| `GROUP_MESSAGES_INDEXER.md` | Server handoff: the batched group-message read the app already uses when present (`POST /group-messages/by-blinded-group-ids`), and the `since` reads that would let the app stop streaming every block for groups |
