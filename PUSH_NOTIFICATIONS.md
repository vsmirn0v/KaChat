...
## Runtime Reliability Gate (Implemented)

Foreground catch-up sync is no longer unconditionally executed after UTXO resubscription in remote-push mode.

Current app behavior:
- Track incoming UTXO txIds that should have push delivery (eligible watched chats only).
- Correlate each txId with APNs receipt in-app/extension flow using a 60-second grace window.
- Reliability transitions:
  - `reliable` -> `unreliable` after 3 consecutive misses.
  - `unreliable` -> `reliable` on first matched txId hit.
- On transition to `unreliable`:
  - Force push re-registration with indexer.
  - Run immediate catch-up sync.
- While `reliable`, app-active and UTXO-subscription-restart catch-up syncs are debounced to 10 minutes to reduce backend load.
